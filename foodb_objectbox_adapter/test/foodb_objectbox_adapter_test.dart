...
Future<ObjectBoxAdapter> getAdapterForIsolate(String dbName,
    {bool persist = false}) async {
  var directory = join(Directory.current.path, 'temp/$dbName');
  final dir = Directory(directory);
  late Store store;
...
class ObjectBoxTestContext extends FoodbTestContext {
  ...
  ObjectBoxTestContext({this.runInIsolate = false});
  ...
  Future<Foodb> db(String dbName,
    ...
    var db = runInIsolate
        ? await getAdapterForIsolate(name)
        : await getAdapter(dbName);
