...
class ObjectBoxAdapterSession implements KeyValueAdapterSession {
  @override
  commit() {
...
class ObjectBoxAdapter implements KeyValueAdapter {
  ...
  @override
  bool delete(AbstractKey<Comparable> key, {KeyValueAdapterSession? session}) {
    final boxType = _getBoxFromKey(key);
    final deleteResult = boxType.remove(store, encodeKey(key));
    return deleteResult == 1 ? true : false;
  }

  @override
  bool deleteMany(List<AbstractKey<Comparable>> keys,
  ...
  @override
  bool clearTable(AbstractKey<Comparable> key,
      {KeyValueAdapterSession? session}) {
    final boxType = _getBoxFromKey(key);
  ...
  @override
  bool destroy({KeyValueAdapterSession? session}) {
  ...
  @override
  Map<T2, Map<String, dynamic>?> getMany<T2 extends AbstractKey<Comparable>>(
      List<T2> keys,
      {KeyValueAdapterSession? session}) {
    if (keys.isEmpty) {
  ...
  @override
  MapEntry<T2, Map<String, dynamic>>? last<T2 extends AbstractKey<Comparable>>(
      T2 key,
      {KeyValueAdapterSession? session}) {
  ...
  @override
  bool put(AbstractKey<Comparable> key, Map<String, dynamic> value,
      {KeyValueAdapterSession? session}) {
    _getBoxFromKey(key).put(store, encodeKey(key), jsonEncode(value));
    return true;
  }

  @override
  bool putMany(Map<AbstractKey<Comparable>, Map<String, dynamic>> entries,
      {KeyValueAdapterSession? session}) {
    _getBoxFromKey(entries.keys.first).putMany(
        store,
        entries
  ...
  @override
  ReadResult<T2> read<T2 extends AbstractKey<Comparable>>(T2 keyType,
      {T2? startkey,
      T2? endkey,
      required bool desc,
      required bool inclusiveEnd,
  ...
  @override
  int tableSize(AbstractKey<Comparable> key,
      {KeyValueAdapterSession? session}) {
