...
mixin _KeyValueChange on _AbstractKeyValue {
  ChangeResult _encodeUpdateSequence(SequenceKey key, UpdateSequence update,
      {bool? includeDocs = false, String? style = 'main_only'}) {
    ...
    if (includeDocs == true) {
      DocHistory docs =
          DocHistory.fromJson(keyValueDb.get(DocKey(key: update.id))!.value);
      result.doc =
    ...
    runZonedGuarded(() async {
      ...
      if (request.feed == ChangeFeed.continuous ||
        ...
        subscription!.onData((entry) async {
          ...
          var changeResult = _encodeUpdateSequence(entry.key, entry.value,
      ...
      if (request.since != 'now') {
        ...
        for (final entry in result.records.entries) {
          ...
          final changeResult = _encodeUpdateSequence(key, update,
              includeDocs: request.includeDocs, style: request.style);
          _results.add(changeResult);
          onResult?.call(changeResult);
          lastSeq = key.key!;
